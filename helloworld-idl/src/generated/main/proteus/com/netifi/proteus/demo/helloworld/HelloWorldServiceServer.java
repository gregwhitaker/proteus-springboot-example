...
public final class HelloWorldServiceServer extends io.netifi.proteus.AbstractProteusService {
  ...
  @javax.inject.Inject
  public HelloWorldServiceServer(HelloWorldService service, java.util.Optional<io.micrometer.core.instrument.MeterRegistry> registry) {
    this.service = service;
    if (!registry.isPresent()) {
      this.sayHello = java.util.function.Function.identity();
    } else {
      this.sayHello = io.netifi.proteus.metrics.ProteusMetrics.timed(registry.get(), "proteus.server", "service", HelloWorldService.SERVICE, "method", HelloWorldService.METHOD_SAY_HELLO);
    }
    ...
    try {
      ...
      switch(io.netifi.proteus.frames.ProteusMetadata.getMethod(metadata)) {
        case HelloWorldService.METHOD_SAY_HELLO: {
          ...
          return service.sayHello(com.netifi.proteus.demo.helloworld.HelloWorldRequest.parseFrom(is), metadata).map(serializer).transform(sayHello);
        }
        default: {
